...
class Rectangle extends MyShape
{
    ...
    {  // 依據corner1,corner2,color,filled畫矩形
    	g.setColor(color);
    	if(filled == true) {
    		g.fillRect(Math.min(corner1.x,corner2.x),Math.min(corner1.y,corner2.y), Math.abs(corner1.x-corner2.x), Math.abs(corner1.y-corner2.y));
    	}else
    		g.drawRect(Math.min(corner1.x,corner2.x),Math.min(corner1.y,corner2.y), Math.abs(corner1.x-corner2.x), Math.abs(corner1.y-corner2.y));
    }
}
...
class Circle extends MyShape
{
    ...
    {   // 依據corner1,corner2,color,filled畫橢圓
    	g.setColor(color);
    	if(filled == true) {
    		g.fillOval(Math.min(corner1.x,corner2.x),Math.min(corner1.y,corner2.y), Math.abs(corner1.x-corner2.x), Math.abs(corner1.y-corner2.y));
    	}else
    		g.drawOval(Math.min(corner1.x,corner2.x),Math.min(corner1.y,corner2.y), Math.abs(corner1.x-corner2.x), Math.abs(corner1.y-corner2.y));
    }
}
...
class MouseHandler extends MouseAdapter implements MouseMotionListener
{
    ...
    {   // 設定新形狀currentShape邊界corner2位置為滑鼠放開座標,
   	...
   	// 新形狀currentShape顏色取自畫板currentColor，實心否狀態取自畫板filledShape
    	...
    	drawPanel.shapeArray.add(drawPanel.currentShape);
    ...
    {   // 設定新形狀currentShape邊界corner2位置為滑鼠拖動座標,
   	...
   	// 將滑鼠於畫板內拖動座標顯示於畫板statusLabel狀態列上
    	drawPanel.statusLabel.setText(String.format("滑鼠拖移位置:(x=%d,y=%d),形狀陣列長度:%d", me.getX(),me.getY(),drawPanel.shapeArray.size()));
    }
}
