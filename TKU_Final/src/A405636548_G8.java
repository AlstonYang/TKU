...
class Board extends JPanel {
	...
	public void initGame() {
		...
		for (int i = 1; i <= noMine; i++) {
			...
			mineList.add(new JButton(String.format("地雷%d", i)));

			add(mineList.get(i - 1));
			JButton mine = mineList.get(i - 1);

			mine.setBounds(rand.nextInt(width), rand.nextInt(height), mine.getPreferredSize().width,
					mine.getPreferredSize().height);
		}
		...
		pacman.setBounds(rand.nextInt(width), rand.nextInt(height), pacman.getPreferredSize().width,
				pacman.getPreferredSize().height);
...
class TAdapter extends KeyAdapter {
	...
	public void keyPressed(KeyEvent e) {
		...
		if (board.inGame) {
			...
			Rectangle r3 = board.pacman.getBounds();
			// 依2維移動量修改矩形的左上角座標值
			r3.x += reqdx;
			r3.y += reqdy;
			// 利用board.pacman的.setBounds方法，修改主角的面積矩形
			board.pacman.setBounds(r3);
...
// 定義主程式類別
public class A405636548_G8 {
	// 說明說明頁籤的文字內容
	static String msg = "G8分組練習: 掃地雷\n" + "  仿小精靈設計，遊戲畫面隨意位置配置一個主角及周圍若干顆固定式地雷。\n" + "  主角可以由鍵盤箭頭作四方位移動，目標是移動到地雷，清除地雷。\n"
			+ "  全部地雷掃除完，遊戲即結束。\n" + "\n" + "提示:\n" + "1.利用踫撞偵測，清除地雷\n" + "2.利用鍵盤監聽器，偵測四方位箭頭事件\n" + "3.主角及地雷皆為按鈕物件\n"
			+ "4.主角物件內含位置及鍵盤監聽方法\n" + "\n" + "可擴充處:\n" + "1.地雷可於某範圍內自主移動\n" + "2.累計主角移動距離，愈短完成任務排名愈前\n" + "3.顯示主角移動軌跡";
